...
const Filter: FC<Props> = ({
	filterItems, title, addFilterProperty, removeFilterProperty,
	filterType, selectMenuItems
}) => {
	...
	const checkBoxItems = React.useMemo(
		() => {
			...
			if (shouldRenderCheckbox) return filterItems.map(checkBoxName => {
				let targetFilter = filtersFromState[title] as string[] | undefined
				let isCheckedByDefault = Boolean(targetFilter && targetFilter.find(
					filterName => filterName === checkBoxName
				))
				return (
					<FilterCheckbox
						key={checkBoxName}
	...
	const selectorItems = React.useMemo(
		() => {
			...
			if (shouldRenderSelect) return selectMenuItems.map(selectMenuItem =>
				<SortSelect
